...
def _update(progress_cb, stage, pct):
    if progress_cb:
...
def translate_segments(segments, progress_cb=None):
        ...
        for attempt in range(3):
            try:
                ...
                if isinstance(result, dict):
                    arr = result.get("segments") or result.get("translations") or list(result.values())[0]
                else:
                    arr = result
                for item in arr:
    ...
    result_segments = []
    for seg in segments:
        result_segments.append({**seg, "translated": translated.get(seg["id"], seg["text"])})
    return result_segments


def generate_tts(segments, tmp_dir, progress_cb=None):
    client = _client()
    total = len(segments)
...
def run_pipeline(youtube_url, job_id, progress_cb=None):
